	...
	//Para secretaria, certificado de promocion
		...
		public List<Registro> findByCedulaEstudiante(String cedula);
			...
			@Query(value="SELECT p.cedula,p.nombre,p.apellido,re.id_registro, re.aporte1, re.aporte2, re.examen_Iquimestre, re.promedio_Iquimestre, "
					+ "re.aporte3, re.aporte4, re.examen_IIquimestre,re.promedio_IIquimestre,re.conducta, re.nota_final, re.examen_supletorio, re.examen_remedial, re.examen_gracia, re.estado, re.fkid_asignatura, re.id_matricula  "
					+ "				 FROM registro re "
					...
					+ "				 where ae.id_empleado= ?1  and pe.id_periodo =?2 and m2.id_malla =?3 and m3.id_modalidad =?4 and c.id_curso =?5 and p2.id_paralelo =?6 and re.fkid_asignatura =?7 "
			...
			List<Registro> filtrocompleto(Long id_empleado, Long id_periodo,Long id_malla , Long id_modalidad, Long id_curso, Long id_paralelo, Long id_asignatura);
			
			
			
			@Query(value=" SELECT p.cedula,p.nombre,p.apellido,re.id_registro, re.aporte1, re.aporte2, re.examen_Iquimestre, "
					+ " re.promedio_Iquimestre, re.aporte3, re.aporte4, re.examen_IIquimestre,re.promedio_IIquimestre, re.conducta, "
					+ " re.nota_final, re.examen_supletorio, re.examen_remedial, re.examen_gracia,re.estado , re.fkid_asignatura, re.id_matricula "
					+ "	 FROM registro re "
					+ "	 inner join matriculas m  on m.id_matricula = re.id_matricula "
					+ "	 inner join estudiantes e on e.id_estudiante = m.id_estudiante "
					+ "	 inner join persona p on p.id_persona = e.id_persona "
