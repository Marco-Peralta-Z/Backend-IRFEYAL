...
public interface IAsistenciaDao extends JpaRepository<Asistencia, Long>{
	...
	@Query(value="SELECT * "
			...
			+ "WHERE asis.id_estudiante = ?1 and c.id_asignatura=?3 and c.id_docente=?2 and asis.estado_asis=true",nativeQuery=true)
	List<Asistencia> obtenerIdEstudiante(Long id_estudiante,Long id_docente,Long idasignatura);
	@Query(value="SELECT * "
			+ "FROM Asistencia asis "
			+ "join clase c on c.id_clase = asis.id_clase "			
			+ "WHERE asis.id_estudiante = ?1 and c.id_asignatura=?2 and asis.estado_asis=true",nativeQuery=true)
	List<Asistencia> obtenerIdEstudianteAdmin(Long id_estudiante,Long idasignatura);

	@Query(value="SELECT * "
			+ "FROM Asistencia asis "
			+ "join clase cls on cls.id_clase = asis.id_clase "
			+ "WHERE  cls.id_modalidad_id_modalidad=?1 and cls.id_periodo_id_periodo=?2 and cls.id_paralelo=?3  and cls.id_curso=?4 and cls.id_docente=?5 and asis.estado_asis=true",nativeQuery=true)
	List<Asistencia> sistenciapdf(Long id_mod, Long id_periodo, Long id_paralelo,Long id_curso,Long docente);

	@Query(value="SELECT * "
			+ "FROM Asistencia asis "
			+ "join clase cls on cls.id_clase = asis.id_clase "
			+ "WHERE  cls.id_modalidad_id_modalidad=?1 and cls.id_periodo_id_periodo=?2 and cls.id_paralelo=?3  and cls.id_curso=?4 and asis.estado_asis=true",nativeQuery=true)
	List<Asistencia> sistenciapdftutor(Long id_mod, Long id_periodo, Long id_paralelo,Long id_curso);
