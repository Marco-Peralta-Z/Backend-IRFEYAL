...
public interface IAsistenciaDao extends JpaRepository<Asistencia, Long>{
	...
	@Query(value="SELECT * "
			+ "FROM Asistencia asis "
			+ "join clase cls on cls.id_clase = asis.id_clase "
			+ "WHERE  cls.id_modalidad_id_modalidad=?1 and cls.id_periodo_id_periodo=?2 and cls.id_paralelo=?3  and cls.id_curso=?4 and cls.id_docente=?5",nativeQuery=true)
	List<Asistencia> sistenciapdf(Long id_mod, Long id_periodo, Long id_paralelo,Long id_curso,Long docente);

	@Query(value="SELECT * "
			+ "FROM Asistencia asis "
			+ "join clase cls on cls.id_clase = asis.id_clase "
			+ "WHERE  cls.id_modalidad_id_modalidad=?1 and cls.id_periodo_id_periodo=?2 and cls.id_paralelo=?3  and cls.id_curso=?4 ",nativeQuery=true)
